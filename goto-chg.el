...
;; Go to Last Change
;;
;; Go to the point of the most recent edit in the buffer.
;; When repeated, go to the second most recent edit, etc.
...
(defvar glc-default-span 8
  "*Don't visit changes less than this many characters from last visit.")
(defvar glc-current-span 8 "Internal variable used in `goto-last-change'.")
(defvar glc-probe-depth 0
  "Internal variable used in `goto-last-change'.
Non-zero between successive `goto-last-change' invocations.")
...
(defun glc-adjust-pos (pos e)
  "Return the \"same\" buffer position as POS after edit E.
Exception: returns nil if POS is closer than `glc-current-span' to the edit E.

Insertion edits before POS return a larger value.
Deletion edits before POS return a smaller value.

The edit E is an entry from the `buffer-undo-list', which see for details."
  (cond ((atom e)                    ; nil==cmd boundary, or, num==changed pos
	...
	((numberp (car e))		; (beg . end)==insertion
	 (glc-adjust-pos2 pos (car e) (car e) (- (cdr e) (car e))))
        ...
        ;;       ((> pos (+ (car e) glc-current-span))
        ;;        (+ pos (- (cdr e) (car e))))
        ;;       (t nil)))
        ((stringp (car e))              ; (string . pos)==deletion
         (glc-adjust-pos2 pos
                          (abs (cdr e))
                          (+ (abs (cdr e)) (length (car e)))
                          (- (length (car e)))))
        ;; (cond ((< pos (- (abs (cdr e)) glc-current-span)) pos)
        ;;       ((> pos (+ (abs (cdr e)) (length (car e))
        ;;                  glc-current-span))
        ;;        (- pos (length (car e))))
        ;;       (t nil)))
...
R is basically a reversed slice from `buffer-undo-list'."
  (if r
      ;; Get pos
...
(defun glc-get-pos (e)
  "If E represents an edit, return a the position where the edit took place.
Return nil if E represents no real change.
E is an entry in `buffer-undo-list'."
...
It does not go to the same point twice even if there have been many edits
there. Set variable `glc-default-span' to control how close \"the same point\".
is. Default span is 8.
...
discarded. See the variable `undo-limit'."
  (interactive "P")
  ...
  (cond ((numberp arg)			; Numeric arg sets span
	...
	((consp arg)			; C-u's multiply previous span by 4
	 (setq glc-current-span (* (abs (car arg)) glc-default-span))
         ;; todo: keep message with "waiting" and "is saved"
	 (message "Current span is %d chars" glc-current-span)))
  ...
  (let (rev				; Reversed (and filtered) undo list
    ...
    ;; Walk back and forth in `buffer-undo-list', each time one step deeper,
