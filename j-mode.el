...
;;     (autoload 'run-j "j-mode.el" "Run J from emacs." t)
...
(defcustom j-path (expand-file-name "~/j602/")
  "The path under which J is installed."
  :type 'directory
  :group 'j)

(defcustom j-command "bin/jee"
  "Command used to invoke J."
  :type 'string
  :group 'j)


(defcustom j-command-args '("~/j602/bin/jwd")
  "Command args J is invoked with."
  :type '(repeat string)
  :group 'j)

(defcustom j-dictionary-url (concat "file://"
                                    (expand-file-name
                                     "help/dictionary/" j-path)) ;"http://www.jsoftware.com/books/help/dictionary/"
  "Base URL of the J dictionary files."
  :type 'string
  :group 'j)

(defcustom j-foreign-help-url (concat  j-dictionary-url "xmain.htm")
  "URL pointing to help for the foreign ('!:') conjunction."
  :type 'string
  :group 'j)

(defcustom j-vocabulary-help-url (concat  j-dictionary-url "vocabul.htm")
...
(defvar j-mode-syntax-table
  (let ((table (make-syntax-table)))
    (modify-syntax-entry ?\{  "."     table) ; punct, not bracket
    (modify-syntax-entry ?\}  "."     table) ; "                "
    (modify-syntax-entry ?\[  "."     table) ; "                "
    (modify-syntax-entry ?\]  "."     table) ; "                "
    (modify-syntax-entry ?\"  "."     table) ; punct, not string
    (modify-syntax-entry ?\.  "w"     table) ; word, not punct
    (modify-syntax-entry ?\:  "w"     table) ; word, not punct
    (modify-syntax-entry ?\(  "(\)"   table) ; open paren
    (modify-syntax-entry ?\)  ")\("   table) ; close paren
  ;; XXX this means 'foo''bar' is treated as *2* sexps, unfortunately this
  ;; is not easily fixed
    (modify-syntax-entry ?\'  "\""     table)
    (modify-syntax-entry ?\\  "."     table) ; punct, not escape
  ;; (modify-syntax-entry ?\'  "\\"     table)
    table))
...
(defun j-end-of-explicit-definition ()
  "Go to the end of the next explicit definition below point."
  (interactive)
  (let ((old-point (point)))
  ...
  (save-match-data
    (let ((type (catch 'done
                  (while (< (point-at-eol) (point-max))
                    (let ((t (j-which-explict-definition)))
                      (and t (throw 'done t)))
                    (forward-line 1)))))
      (ecase type
        ((nil) nil)
...
(defun j-beginning-of-explicit-definition ()
  "Go to the start of the next explicit definition above point."
  (interactive)
  ...
  (save-match-data
    (catch 'done
      (while (< (point) (point-min))
        (forward-line -1)
        (and (j-which-explict-definition)
             (throw 'done t))))))
...
(defvar j-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (if j-no-generic-defun-functions
        (progn
          (define-key map "\e\C-a"   'j-beginning-of-explicit-definition)
          (define-key map "\e\C-e"   'j-end-of-explicit-definition)
          (defun j-mark-defun ()
          ...
          (define-key map [(control meta h)]   'j-mark-defun))
      (fset 'j-mark-defun 'mark-defun)
      (fset 'j-beginning-of-explicit-definition 'beginning-of-defun)
      (fset 'j-end-of-explicit-definition 'end-of-defun))
    (define-key map "\C-c\C-z" 'run-j)
    (define-key map "\C-c\C-c" 'j-execute-buffer)
    (define-key map "\e\C-x"   'j-execute-explicit-definition)
    (define-key map "\C-c\C-r" 'j-execute-region)
    (define-key map "\C-c\C-l" 'j-execute-line)
    (define-key map "\C-c\C-df" 'j-browse-help-foreign)
    (define-key map "\C-c\C-dv" 'j-browse-help-vocabulary)
    (define-key map "\C-c\C-i" 'j-describe-voc)
    map)
  "Keymap for J mode.")
...
(defun j-mode ()
  ...
  4. Jumping to definitions (via `imenu'),
     marking (\\[j-mark-defun]) and moving
     up (\\[j-beginning-of-explicit-definition]) and
     down (\\[j-end-of-explicit-definition]) explicit
     definitions.
...
(defvar inferior-j-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map "\C-c\C-i" 'j-describe-voc)
    map)
  "Keymap for Inferior J mode.")

(define-derived-mode inferior-j-mode comint-mode "Inferior J"
  ;; Major mode for interacting with an inferior J process.
  ...
  (or (get-process "J")
      (progn
        (with-current-buffer (apply 'make-comint
                                    "J"
                                    (expand-file-name j-command j-path)
                                    nil
                                    j-command-args)
          (inferior-j-mode))
        ;; (make-local-variable 'font-lock-defaults)
...
(defun j-execute-buffer (arg)
  ...
  Possible values for ARG:
    ...
    3  (i.e. #b11): continue-on-error  display
...
(defconst j-voc-to-doc-alist
  ...
  "Mapping of J vocabulary items to HTML help pages.")
...
(defun j-describe-voc (voc)
  "Look-up J vocabulary item at point on-line in the official J docs.
Prompt when no recognized vocabulary item under point."
  (interactive (list (let ((voc-at-point
                       ...
                       (completing-read
                        "Look up an item in the J dictionary: "
                        j-voc-to-doc-alist nil nil ; don't require match -- foreign!
                        voc-at-point
                        'j-describe-function-history
                        voc-at-point))))
  (let ((page (cond ((string-match j-foreign-regexp voc)
                     (j-foreign-to-doc voc))
                    ((assoc voc j-voc-to-doc-alist)
                     (cdr (assoc voc j-voc-to-doc-alist)))
                    (t (message "Don't know about %s, trying Google" voc)
                       nil))))
    (browse-url (if page (concat j-dictionary-url page) (j-hack-google-url voc)))))
